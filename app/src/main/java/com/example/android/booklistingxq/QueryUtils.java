...
public final class QueryUtils {
    ...
    private static final String NO_PRICE = "FREE";
        ...
        try {
            ...
            if (jsonObject.has("items")) {
                ...
                for (item = 0; item < arrayItems.length(); item++) {
                    ...
                    JSONObject SaleinfoObject , listPriceobject ;
                    // get "listprice" JSONObject
                    // extract "price" for title of a book
                    String price = "";
                    String currency ="" ;

                    if (bookInfo.has("saleInfo")) {
                        SaleinfoObject = bookInfo.getJSONObject("saleInfo");
                        if (SaleinfoObject.has("listPrice")){
                            listPriceobject = SaleinfoObject.getJSONObject("listPrice");


                            try {
                                price = listPriceobject.getString("amount");
                                currency = listPriceobject.getString("currencyCode");
                            } catch (JSONException exc_07) {
                                price = NO_PRICE;
                    ...
                    if (bookInfo.has("accessInfo")) {

                        accessinfoObject = bookInfo.getJSONObject("accessInfo");

                       if (accessinfoObject.has("webReaderLink")){
                       //     listPriceobject = SaleinfoObject.getJSONObject("listPrice");

                            try {
                                url = accessinfoObject.getString("webReaderLink");
                            } catch (JSONException exc_07) {
                                url = NO_url;
                    ...
                    Book book = new Book(title, authors, description, imageLink , price , currency , url);
